...
/*   Updated: 2015/01/27 16:11:13 by tmertz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	wolf->cam_x = 2.0 * (double)i / (double)MAP_W - 1.0;
	...
	wolf->delta_dist_x = sqrt(1.0 + (wolf->raydir_y * wolf->raydir_y)
							/ (wolf->raydir_x * wolf->raydir_x));
	wolf->delta_dist_y = sqrt(1.0 + (wolf->raydir_x * wolf->raydir_x)
...
void		ft_wolf3d_direction(t_wolf *wolf)
{
	...
	{
		wolf->step_x = -1;
		wolf->side_dist_x = (wolf->raypos_x - (double)wolf->map_x)
							* wolf->delta_dist_x;
	}
	else
	{
		wolf->step_x = 1;
		wolf->side_dist_x = ((double)wolf->map_x + 1.0 - wolf->raypos_x)
			* wolf->delta_dist_x;
	}
	if (wolf->raydir_y < 0)
	{
		wolf->step_y = -1;
		wolf->side_dist_y = (wolf->raypos_y - (double)wolf->map_y)
							* wolf->delta_dist_y;
	}
	else
	{
		wolf->step_y = 1;
		wolf->side_dist_y = ((double)wolf->map_y + 1.0 - wolf->raypos_y)
			* wolf->delta_dist_y;
	}
}
...
void		ft_wolf3d_draw_col(t_wolf *wolf, int j)
{
	...
	if (draw_end > MAP_H)
		draw_end = MAP_H;
	ft_draw_walls(draw_start, draw_end, wolf, j);
}
