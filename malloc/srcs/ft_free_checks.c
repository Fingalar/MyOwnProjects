...
/*   Updated: 2015/04/29 13:25:23 by tmertz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			check_if_valid_address(void *ptr)
{
	...
	while (map)
	{
		if (ptr > map && ptr < map + g_maps.total_size_t
				&& ptr - SIZE_HEADER >= map)
			return (1);
	...
	while (map)
	{
		if (ptr > map && ptr < map + g_maps.total_size_s
				&& ptr - SIZE_HEADER >= map)
			return (1);
...
void		check_if_empty_map(t_header *header)
{
	...
	if (0 == header->prev_size && END == next_header->flag)
	{
		...
		g_maps.size_allocs -= header->size + SIZE_HEADER * 2;
